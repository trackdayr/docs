/* Add copy button to code blocks */
const copyIcon = `<svg aria-hidden="true" height="16" viewBox="0 0 16 16" width="16">
                    <path fill="currentColor" fill-rule="evenodd" d="M0 6.75C0 5.784.784 5 1.75 5h1.5a.75.75 0 010 1.5h-1.5a.25.25 0 00-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 00.25-.25v-1.5a.75.75 0 011.5 0v1.5A1.75 1.75 0 019.25 16h-7.5A1.75 1.75 0 010 14.25v-7.5z"></path>
                    <path fill="currentColor" fill-rule="evenodd" d="M5 1.75C5 .784 5.784 0 6.75 0h7.5C15.216 0 16 .784 16 1.75v7.5A1.75 1.75 0 0114.25 11h-7.5A1.75 1.75 0 015 9.25v-7.5zm1.75-.25a.25.25 0 00-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 00.25-.25v-7.5a.25.25 0 00-.25-.25h-7.5z"></path>
                  </svg>`;

const copiedIcon = `<svg aria-hidden="true" height="16" viewBox="0 0 16 16" width="16" data-view-component="true">
                      <path fill="currentColor" fill-rule="evenodd" d="M13.78 4.22a.75.75 0 010 1.06l-7.25 7.25a.75.75 0 01-1.06 0L2.22 9.28a.75.75 0 011.06-1.06L6 10.94l6.72-6.72a.75.75 0 011.06 0z"></path>
                    </svg>`;

const buttonClasses =
  "p-2 bg-slate-800 rounded-md text-slate-200 group-hover:opacity-100 opacity-0 transition-opacity duration-100 absolute right-2 top-2 border border-slate-600 hover:border-slate-200 hover:bg-slate-700";
const buttonSuccessClasses =
  "p-2 bg-slate-800 rounded-md text-green-500 opacity-100 absolute right-2 top-2 border border-green-500 transition-opacity duration-100";

let blocks = document.querySelectorAll("pre");

blocks.forEach((block) => {
  // only add a button if browser supports Clipboard API
  if (navigator.clipboard) {
    block.classList.add("group", "relative");
    let button = document.createElement("button");
    button.className = buttonClasses;
    button.innerHTML = copyIcon;

    button.addEventListener("click", copyCode);
    block.appendChild(button);
  }
});

async function copyCode(event) {
  const button = event.currentTarget;
  const pre = button.parentElement;
  let code = pre.querySelector("code");
  let text = code.innerText;

  await navigator.clipboard.writeText(text);

  button.innerHTML = copiedIcon;
  button.className = buttonSuccessClasses;

  setTimeout(() => {
    button.className = buttonClasses;

    // if button is not hovered change icon after 100 milliseconds
    let time = button.parentElement.matches(":hover") ? 0 : 100;
    console.log(time);
    setTimeout(() => {
      button.innerHTML = copyIcon;
    }, time);
  }, 1000);
}
